...
nav_p.x01 = [L; L; 0;0];               % Start position for agent 1
nav_p.xd1 = [-L-offset; -L; 0;0];      % Goal position for agent 1

nav_p.x02 = [-L; -L; 0;0];            % Start position for agent 2
nav_p.xd2 = [L; L+offset; 0;0;];        % Goal position for agent 2

nav_p.x03 = [L; -L; 0;0];             % Start position for agent 3
nav_p.xd3 = [-L; L+offset; 0;0];      % Goal position for agent 3

nav_p.x04 = [-L; L; 0;0];           % Start position for agent 4
nav_p.xd4 = [L; -L-offset; 0;0];    % Goal position for agent 4
...
x_euler(1,1:4,1) = nav_p.x01;
x_euler(1,1:4,2) = nav_p.x02;
x_euler(1,1:4,3) = nav_p.x03;
x_euler(1,1:4,4) = nav_p.x04;
u_euler = zeros(M, 2, 4);
...
for iter = 1:M
    ...
    for i = 1:4
        [x_euler(iter, :, i), u_euler(iter, :, i)] = forwardEuler_multiagent(nav_p, deltaT, ctrl_multiplier, ...
            @doubleIntegrator_multiagent, gradDensityHandles, c{1}, c{2}, c{3}, c{4}, dbl_int_p, x_temp{i}, i);
...
for i = 1:4
    subplot(2, 2, i);
    plot(1:M, squeeze(x_euler(1:M, 1:2, i)),'-', 'LineWidth', 2);
    ylabel('positions');
    xlabel('timesteps');
...
for i = 1:4
    subplot(2, 2, i);
    plot(1:M, squeeze(u_euler(1:M, 1, i)), 'LineWidth', 2);
    ylabel('velocity');
    xlabel('timesteps');
...
for jj = 1:skip_rate:M
    ...
    for i = 1:4
        ...
        if size(new_position, 2) == 2
            ...
            plotCircle(squeeze(x_euler(jj, 1, i)), ...
